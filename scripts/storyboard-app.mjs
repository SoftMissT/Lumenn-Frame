...
// O canvas pode recuar até 20% para dar visão geral de grafos extensos.
// Os alvos interativos mantêm vector-effect e a barra oferece retorno a 100%.
const ZOOM_STEP = GRAPH_ZOOM_STEP;
...
export class LumennGraphApp extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  async _prepareContext() {
    ...
    if (this.#mode === "live" && activeGraph?.activeNodeId) {
      for (const e of activeGraph.edges ?? []) {
        if (
          ...
          e.from === activeGraph.activeNodeId
        )
          navigable.add(e.to);
      }
    }
    ...
    const nodes = (activeGraph?.nodes ?? []).map((n) => {
      ...
      const out = {
        ...n,
        color: normalizeHexColor(n.color, defaultColors[n.type] ?? "#73707c"),
        width: w,
        height: h,
    ...
    return {
      ...
      label: {
        ...
        audioRoutingNote: L("LUMENN_FRAME.Inspector.AudioRoutingNote"),
  ...
  async _onRender(context, options) {
    ...
    if (
      !this._workspaceAuto &&
      LumennSettings.get("openInWorkspace") &&
      !this.#expanded
    ) {
  ...
  #bindNode(root, node) {
    ...
    node.addEventListener("pointerdown", (e) => {
      ...
      const up = async (ev) => {
        ...
        const updated = await LumennGraphStore.updateNode(
          this.#graphId,
          node.dataset.id,
          {
            position: next,
  ...
  #bindCamera(root) {
    ...
    viewport.addEventListener(
      ...
      (e) => {
        ...
        const delta = wheelPanDelta(e, viewport.clientHeight);
        this.#camera.panX -= delta.x;
        this.#camera.panY -= delta.y;
        this.#applyCamera(root);
      },
      { passive: false },
    );
  ...
  #nodeBounds(nodeId, positionOverride = null, graph = null) {
    const node =
  ...
  #portAnchor(nodeId, port, positionOverride = null, graph = null) {
    const bounds = this.#nodeBounds(nodeId, positionOverride, graph);
    if (!bounds) return { x: 0, y: 0 };
    const { x, y, width: w, height: h } = bounds;
    if (port === "flow-out") return { x: x + w, y: y + h * 0.3 };
    if (port === "flow-in") return { x, y: y + h * 0.3 };
    if (port === "audio-out") return { x: x + w, y: y + h * 0.62 };
    return { x, y: y + h * 0.62 };
  }
  ...
  #edgeGeometry(edge, graph, positionOverrides = null) {
    ...
    const fromBounds = this.#nodeBounds(edge.from, fromPosition, graph);
    const toBounds = this.#nodeBounds(edge.to, toPosition, graph);
    ...
    const a = directionalRectAnchor(fromBounds, toCenter, "right");
    const b = directionalRectAnchor(toBounds, fromCenter, "left");
    const hasReverse =
      edge.type === "flow" &&
      graph.edges.some(
        (candidate) =>
  ...
  #drawEdges(root) {
    ...
    for (const e of graph?.edges ?? []) {
      ...
      if (e.type === "flow") {
        if (selected) path.style.stroke = "#ffffff";
        else {
          const gradient = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "linearGradient",
          );
          const gradientId = `lf-flow-${e.id}`;
          ...
          start.setAttribute(
            "stop-color",
            from.id === graph.activeNodeId ? "#ffd166" : "#f0a321",
          );
          ...
          end.setAttribute("stop-color", "#8b6a28");
        ...
        // Cabo estéreo: vermelho = canal direito/saída, branco = canal
        // esquerdo/entrada. As duas linhas ficam paralelas e legíveis em zoom.
        path.style.stroke = selected ? "#ff7b7b" : "#ef4444";
        path.setAttribute("stroke-width", selected ? 3.2 : 2.4);
        path.setAttribute("opacity", selected ? 1 : 0.9);
        path.setAttribute("transform", `translate(${nx * 2.5} ${ny * 2.5})`);
        const left = path.cloneNode(true);
        left.classList.add("lf-edge-audio-left");
        left.style.stroke = "#ffffff";
        left.setAttribute("transform", `translate(${-nx * 2.5} ${-ny * 2.5})`);
        svg.appendChild(left);
      }
      svg.appendChild(path);

      for (const [edgeEndName, point] of [
        ["start", edgeStart],
        ["end", edgeEnd],
      ]) {
        const dock = document.createElementNS(
          "http://www.w3.org/2000/svg",
          "circle",
        );
        dock.setAttribute("cx", point.x);
        dock.setAttribute("cy", point.y);
        dock.setAttribute("r", e.type === "flow" ? 3.5 : 3);
        dock.setAttribute("class", `lf-edge-dock lf-edge-dock-${e.type}`);
        dock.setAttribute("data-edge-id", e.id);
        dock.setAttribute("data-edge-end", edgeEndName);
        dock.setAttribute("vector-effect", "non-scaling-stroke");
      ...
      if (
        e.type === "flow" &&
        LumennSettings.get("showTransitionLabels") &&
    ...
    for (const edge of graph.edges ?? []) {
      ...
      for (const element of elements) {
        ...
        if (element.tagName.toLowerCase() === "circle") {
          const point =
            element.dataset.edgeEnd === "start" ? geometry.start : geometry.end;
        ...
        if (
          edge.type !== "audio" ||
          element.classList.contains("lf-edge-hit") ||
          element.classList.contains("lf-edge-audio-role")
        )
          continue;
        const side = element.classList.contains("lf-edge-audio-left")
          ? -2.5
          : 2.5;
  ...
  #scheduleEdgeDraw(
    ...
    this.#edgeFrame = requestAnimationFrame(() => {
      ...
      this.#pendingEdgeNodes.clear();
      const graph =
        this.#edgeGraphSnapshot ?? LumennGraphStore.getGraph(this.#graphId);
      this.#edgeGraphSnapshot = null;
      this.#updateIncidentEdges(root, graph, overrides);
    });
  }
  ...
  #edgeLabelText(e) {
    ...
    const sceneLbl =
      t.scene?.type === "fade"
        ? L("LUMENN_FRAME.Edge.Fade")
        : L("LUMENN_FRAME.Edge.Cut");
    const modeKey = {
  ...
  #highlightTarget(ev) {
    const target = this.#findConnectTarget(ev.clientX, ev.clientY);
  ...
  #findConnectTarget(clientX, clientY, connectState = this.#connectState) {
    const state = connectState;
    if (!state) return null;
    const expected = state.port === "flow-out" ? "flow-in" : "audio-in";
    const direct = document
      .elementFromPoint(clientX, clientY)
      ?.closest?.(`[data-port="${expected}"]`);
    if (direct?.dataset?.node && direct.dataset.node !== state.nodeId)
      return { nodeId: direct.dataset.node, port: expected };

    // A porta visual continua pequena, mas o lado compatível inteiro do nó é
    // uma área de encaixe. Isso evita exigir precisão de pixel no zoom baixo.
    const node = document
      .elementFromPoint(clientX, clientY)
      ?.closest?.(".lf-node");
    if (node?.dataset?.id && node.dataset.id !== state.nodeId) {
      const port = node.querySelector(`[data-port="${expected}"]`);
      if (port) return { nodeId: node.dataset.id, port: expected };
    }

    let best = null;
    let bestDistance = Infinity;
    this.element
      .querySelectorAll(`[data-port="${expected}"]`)
      .forEach((candidate) => {
        if (candidate.dataset.node === state.nodeId) return;
        ...
        if (distance <= pad && distance < bestDistance) {
          best = { nodeId: candidate.dataset.node, port: expected };
  ...
  #finishConnect(ev) {
    ...
    const hit = this.#findConnectTarget(ev.clientX, ev.clientY, state);
    const toNodeId = state.targetNodeId ?? hit?.nodeId ?? null;
    const toPort = state.targetPort ?? hit?.port ?? null;
  ...
  #inspectorChange(e) {
    ...
    if (
      ...
      val = Number.isFinite(Number(val)) ? Number(val) : null;
    if (
      typeof val === "number" &&
      /Duration|duration|fadeIn|fadeOut/.test(path)
    )
      val = Math.max(0, Math.min(10000, val));
    if (typeof val === "number" && path.endsWith("volume"))
  ...
  #inspectorAction(e) {
    ...
    if (action === "focus-node") {
      ...
      const targetId = root.querySelector(
        '[data-insp-pick="flowTarget"]',
      )?.value;
      if (targetId && this.#selected.kind === "node") {
        const edge = {
          id: `edge_${foundry.utils.randomID(16)}`,
          type: "flow",
          from: this.#selected.id,
          to: targetId,
        };
        LumennGraphStore.addEdge(this.#graphId, edge).then((created) => {
          if (created) {
      ...
      const audioId = root.querySelector(
        '[data-insp-pick="audioAttach"]',
      )?.value;
  ...
  #audioTransitionFor(graph, flowEdge) {
    const base = {
      mode: "auto",
      ...
      fadeOutDuration: 0,
      curve: "linear",
      ...(flowEdge.transition?.audio ?? {}),
    };
    // Uma associação AUDIO pode configurar o crossfade diretamente. O valor
    // do FLOW continua sendo o fallback para preservar grafos antigos.
    for (const audioEdge of graph?.edges ?? []) {
      if (audioEdge.type !== "audio" || audioEdge.to !== flowEdge.to) continue;
      Object.assign(base, audioEdge.transition?.audio ?? {});
    }
    return base;
  }
